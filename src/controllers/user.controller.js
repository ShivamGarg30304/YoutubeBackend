...
const loginUser = asyncHandler(async (req, res) => {
  ...
  // check wheather the email or username is given or not
  console.log(username);
  console.log(password);
  if (!username && !email) {
    throw new ApiError(400, "username or email is required");
  }
  // find the user in the database using the email or the username
  const user = await User.findOne({
    $or: [{ username }, { email }],
  });
  // if user not exits then return to the user
  if (!user) {
    throw new ApiError(404, "user does not exist");
  }
  // check wheather the password is correct or not
  const isPasswordValid = await user.isPasswordCorrect(password);
  // if the password is not valid return the wrong password error to the user
  if (!isPasswordValid) {
    throw new ApiError(401, "Invalid user credentials");
  }

  // create the method for the generation of access and refresh tokens
